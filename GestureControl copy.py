...
cube = Entity(model="hammer.obj", texture="./Basic1.png",
              scale=0.2, double_sided=True)
...
def update():
    ...
    if len(lmList) != 0:
        ...
        if noHands == 1:
            ...
            # if fingers[0] == 1:
            lengthR, imgr, lineInfo = detector.findDistance(
                4, 8, lmList, lmList, img, r=7)
            # lengthR = round(lengthR, 2)
            if lengthR <= clickBufferVal:
                clickState = True
                # print(lineInfo[4], lineInfo[5])
                xc = lineInfo[4]
                yc = lineInfo[5]
                rot["initials"] = rot["currents"]
                rot["currents"] = [xc, yc]
            ...
            # print(rot["initials"], rot["currents"])

            if clickState == True and rot["initials"] != []:
                netX = rot["currents"][0] - rot["initials"][0]
                netY = rot["currents"][1] - rot["initials"][1]
                print(netX, netY)
                # netX = netX/rotSmooth
                # netY = netY/rotSmooth
                cube.rotation_y = cube.rotation_y + netX
                cube.rotation_x = cube.rotation_x - netY
